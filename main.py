# Render.com အတွက် ပြင်ဆင်ထားသော Code
# Owner: Zawgyi
...
# ---------- Configuration ----------
BOT_TOKEN = "8351350268:AAFVrxdthzfJyHgGgI0OmC20spcmkyq1g6A"
ADMIN_IDS = {"7246565833"}

# Render.com အတွက် URL ကို အောက်ပါအတိုင်း သတ်မှတ်ပါ
# Render မှာ Deploy လုပ်ပြီးရင် ရလာတဲ့ URL ကို ဒီနေရာမှာ ထည့်ပါ
# ဥပမာ: https://your-app-name.onrender.com
...
@flask_app.route('/')
def index():
    return f"""<!DOCTYPE html><html><head><title>ViralStream</title>
...
p{{color:#666;line-height:1.6;margin-bottom:8px}}code{{background:#1e1e1e;padding:3px 8px;border-radius:4px;color:#e63946}}
.owner{{color:#555;font-size:.7rem;margin-top:20px;border-top:1px solid #1a1a1a;padding-top:12px}}
.owner span{{color:#e63946;font-weight:700}}</style></head>
...
<div class="owner">👤 Owner: <span>{OWNER_NAME}</span> {OWNER_USERNAME}</div>
</div></body></html>""", 200
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        ...
        f"🤖 <b>Bot Owner</b>: {OWNER_NAME} {OWNER_USERNAME}"
    )
...
async def cmd_addall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def notify_one(uid, pts, total_after):
        msg = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎉 Admin {OWNER_NAME} မှ လက်ဆောင်ပေးသည်!\n"
            f"👥 Refer လုပ်ပြီး ထပ်ပိုမို points ရယူပါ\n"
            f"💳 Points ဝယ်ယူလိုပါက {OWNER_USERNAME}"
        )
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in MODE_MAP:
        ...
        if not is_admin(user_id):
            await update.message.reply_text(
                f"❌ <b>Admin သာ Links ဖျက်နိုင်သည်</b>\n\nဖျက်ရန် Admin ထံ ဆက်သွယ်ပါ {OWNER_USERNAME}",
                parse_mode="HTML"
            )
            return
        ...
        await update.message.reply_text(
            ...
            f"💳 <b>Bot အသုံးပြုနိုင်ရန် points များ ဝယ်ယူလိုပါက</b> {OWNER_USERNAME}\n\n"
            f"👤 <b>Bot Owner</b>: {OWNER_NAME} {OWNER_USERNAME}\n\n"
            "<b>Admin commands:</b>\n"
